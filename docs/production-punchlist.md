...
| K03 | REVIEW | Qualified DRAM↔NVMe restore/eviction works under pressure without blocking decode | Contract candidate `69895e0` defines aligned 493/501-block direct extents, registered-buffer io_uring ownership, restore dedup/cancel, shared catalog epochs, tier scheduling, segment cleaning, and matched decode-isolation evidence; implementation is intentionally absent. The retained blocking restore workers use read-only snapshots at `535a8d6`, and startup now rejects overlapping/out-of-bounds live extents and allocates after physical EOF at `de2d43a`, but still blocks and retains private catalogs | Fable reviews of the direct-tier, single-writer, and catalog-integrity handoffs, then direct-format/CPU state proof |
...
| K05 | OPEN | Prefix cache survives restart, corruption, torn journal, cache thrash, pinned pressure, and DCP posture changes end to end | Candidate `d0ac1d3` adds a deterministic integrated CPU lifecycle proof. Corrections through `de2d43a` bind restore identity, make admission/release transactions atomic, fail-stop uncertain writes, reject multiple writers and same-key logical conflicts, retain exact revisions/MTP capability, reject corrupt complete journal tails, repair only validated short tails, reject overlapping/truncated live catalogs at startup, and preserve physical append-only allocation. All await review, and no model/GPU/direct-I/O lifecycle exists | Preserve the CPU fixture and correction proofs while implementing accepted K03/K04 and C05 paths, then run the integrated cold/warm fault matrix |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| D01 | PASS | Local format, tests, Clippy, FFI checks, deterministic fixtures, review provenance, and pinned tokenizer proof pass | Latest full local run: 257 Rust tests, including live catalog overlap/bounds rejection and physical-EOF append preservation, validated short-tail repair followed by recoverable append, complete journal-tail corruption rejection, shared durable dedup/MTP upgrade/collision-safe replay, two-rank atomic MTP registration, prefix/residency coherence, exclusive durable writer/read-only snapshots, fail-stop durable writes, serving rollback/fatal cleanup, C64/MTP6 event staging, captured-shape prefill, restore identity, HBM admission, and all prior CPU fixtures, passed; all 52 then-present handoffs verified in that full run, and the new catalog-integrity handoff separately passes `review-proof`; accepted-review classification still requires the exact candidate, every pinned input hash, any required result path, and a distinct review artifact; the external tokenizer was not rerun because its directory variable was unset, and its pinned fixture is unchanged | Keep green at every milestone |
