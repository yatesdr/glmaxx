...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| S01 | PASS | CPU scheduler supports bounded continuous batching, weighted tenant fairness, prefix-pending admission, and collective-safe cancellation | Scheduler/serving tests include `2f7d0ce` for retryable C64 completion, `14b97a2` for counted release, `2ff0ac1` for consuming every failed selection, and `6535248` for fixed-capacity success publication plus all-or-nothing multi-user terminal/cancellation cleanup. All dedicated Fable handoffs await review | Resolve the CPU transaction reviews, then re-prove with the real executor |
