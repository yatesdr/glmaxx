...
Baseline audited: `271d1f40529c5f2bd1ac774c385676782f482424`
...
| C02 | REVIEW | Immutable row input carries prompt IDs/positions, sampling tuple, seed/RNG, MTP posture, limits, and page-delta binding to all ranks | CPU ABI `d1f9825` hashes exact row/prompt/sampling state. Integration `e1d51ce` constructs it from retained serving state, shares one immutable object with four ranks, independently verifies it on each rank thread, and binds its hash into every receipt. Final RNG output/commit and device consumption remain absent | Adversarial integrated CPU review, then `StepOutput.v2` counters and SM120 executor consumption |
| C03 | REVIEW | Scheduler, prefix leases, active KV IDs, rank uploads, commit, rollback, and ID quarantine form one transaction | `f480ef1` makes the active table mandatory; `271d1f4` adds canonical deltas/mirrors; `e1d51ce` initializes four persistent mirrors and acknowledges admission, reservation, commit/rollback, and removal generations with global/rank-local digests. Clone rollback, device upload acknowledgment, physical-ID quarantine, and cache-only cleanup remain absent | Review integrated CPU transaction, then fixed undo/quarantine and CUDA stream-visible receipts |
...
| C09 | REVIEW | Distributed greedy, bounded top-k/top-p, mass, residual, and bonus sampling execute without full-vocabulary gather | Contract candidate `7c71818` freezes vocabulary padding, filters, SplitMix counter draws, MTP proposal/accept/residual/bonus schedule, composite routes, and `StepOutput.v2`. Retained CPU primitives exist, and correction `b235dc8` makes greedy reject a globally all-masked row while accepting rank-local masked partitions; the reviewed execution ABI, wire routes, trace, and engine integration remain absent | Fable review of `docs/fable-distributed-sampling-abi-v1-handoff.md` and the greedy correction handoff, then complete the CPU proof |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| D01 | PASS | Local format, tests, Clippy, FFI checks, deterministic fixtures, review provenance, and pinned tokenizer proof pass | Latest full local run: 278 Rust tests, adding page-granular append equivalence, all 448 tail/depth reservations, canonical page-delta reconstruction, digest/owner/arena/generation tamper rejection, plus all prior active-serving, durable-cache, scheduler, tokenizer, format, and reference fixtures; workspace formatting, Clippy, FFI checks, and all 66 handoffs passed. The external tokenizer proof was not rerun because its directory variable was unset and its pinned fixture is unchanged | Keep green at every milestone |
