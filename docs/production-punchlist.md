...
| C03 | REVIEW | Scheduler, prefix leases, active KV IDs, rank uploads, commit, rollback, and ID quarantine form one transaction | `docs/serving-page-transaction-v1.md`; handoff `61eb0b9`. CPU corrections `2f7d0ce` and `14b97a2` separately make scheduler batch completion and prefix release all-or-nothing and retryable on failure, but do not implement the wider multi-request page/rank transaction | Resolve active-page, scheduler/prefix atomicity, and transaction reviews, then implement the bounded cross-component undo log |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| S01 | PASS | CPU scheduler supports bounded continuous batching, weighted tenant fairness, prefix-pending admission, and collective-safe cancellation | Scheduler/serving tests include candidate `2f7d0ce` for retryable C64 batch completion and `14b97a2` for counted all-or-nothing prefix release with lease-preserving retry. Both dedicated Fable handoffs await review | Resolve both atomicity reviews, then re-prove with the real executor |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| D01 | PASS | Local format, tests, Clippy, FFI checks, deterministic fixtures, review provenance, and pinned tokenizer proof pass | Latest full local run: 237 Rust tests, including all-or-nothing prefix release and scheduler completion, captured-shape prefill progress, exact asynchronous restore identity, atomic multi-victim HBM admission, and all prior CPU fixtures, passed; all 38 then-present handoffs verified in that full run, and the new prefix-release handoff separately passes `review-proof`; accepted-review classification also requires the exact candidate, every pinned input hash, any required result path, and a distinct review artifact; external tokenizer was not rerun because its directory variable was unset, and its pinned fixture is unchanged from the prior proof | Keep green at every milestone |
