...
|---|---|---|---|---|
| C01 | REVIEW | Corrected NVFP4 manifest ABI, EXL3 source projection, and EXL3 warp-decode gates accepted | Corrective handoffs at `0edfc8d`; no acceptance tokens are present. Prior manifest findings `m5` and `m7` now have atomic-publication and bit-exact-scale CPU corrections with dedicated handoffs at `aaeffea` and `13f0c59`, also awaiting review | Fable re-review of the three r2 handoffs and the two narrow CPU corrections |
...
| K05 | OPEN | Prefix cache survives restart, corruption, torn journal, cache thrash, pinned pressure, and DCP posture changes end to end | Candidate `d0ac1d3` adds a deterministic integrated CPU proof joining three-page MTP publication, torn-journal restart, prefix reuse, bounded restore saturation, one-page HBM/DRAM pressure, pinned fail-closed behavior, HBM→DRAM→NVMe movement, COW tails, speculative rollback/partial commit, cleanup, and corrupt-payload rejection; no model/GPU/direct-I/O lifecycle | Preserve the CPU fixture while implementing accepted K03/K04 and C05 paths, then run the integrated cold/warm fault matrix |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| D01 | PASS | Local format, tests, Clippy, FFI checks, deterministic fixtures, review provenance, and pinned tokenizer proof pass | Latest full local run: 230 Rust tests, including exhaustive indexer-scale exponent boundaries and the deterministic integrated cache-lifecycle fixture, passed; all 29 then-present handoffs verified in that full run. The later atomic-publication candidate separately passed all 63 `glm-format` tests, workspace Clippy, and its handoff proof; accepted-review classification also requires the exact candidate, every pinned input hash, any required result path, and a distinct review artifact; external tokenizer was not rerun because its directory variable was unset, and its pinned fixture is unchanged from the prior proof | Keep green at every milestone |
...
| D04 | OPEN | Immutable results index covers every accepted CPU, GPU, quality, capacity, and benchmark artifact | Fail-closed v2 verifier proves candidate hashes for all 31 pinned handoffs, automatically ingests declared result paths when they appear, and rejects self-review or token-only acceptance artifacts that omit candidate/input attestations; most required result artifacts still do not exist | Append only provenance-complete records and supply each review artifact explicitly |
