...
Baseline audited: `f480ef179ec7088005b1dbcdc04be113c289974d`
...
| C03 | REVIEW | Scheduler, prefix leases, active KV IDs, rank uploads, commit, rollback, and ID quarantine form one transaction | Design `e7bc477`; CPU serving integration `f480ef1` now makes the active table mandatory, attaches exact restored records, pre-reserves every selected row, commits consensus counts, discards failed candidates, removes terminal/cancelled rows before prefix release, and proves no worker launch on capacity failure. Rank deltas/uploads, fixed undo, device acknowledgments, and ID quarantine remain absent; dedicated review pending | Fable active-page transaction verdict, then fixed-capacity rank-delta/acknowledgment implementation |
| C04 | REVIEW | Active sequence page table and page-aligned C64/MTP6 arena arithmetic accepted | CPU implementations `3404e07` and `c33648a`; corrections through `6a5c574` bind active prefix identity, and `f480ef1` integrates target/draft capacity into coordinator admission, step reservation, terminal cleanup, and exact 1M-boundary execution. Clone/per-token mechanics remain non-production. Separate handoffs await review | Fable page-table, cache-budget, sequence-removal, prefix-binding, and active-transaction verdicts |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| D01 | PASS | Local format, tests, Clippy, FFI checks, deterministic fixtures, review provenance, and pinned tokenizer proof pass | Latest full local run: 273 Rust tests, including mandatory coordinator active pages, exact MTP0/MTP6-capable 1M-boundary accounting and cleanup, pre-worker capacity rejection, dynamic MTP tail bounding, cancellation-before-peer cleanup, active-prefix identity, queue-independent backend cancellation, and all prior durable-cache, serving, scheduler, tokenizer, format, and reference fixtures; workspace formatting, Clippy, FFI checks, and 65 then-present handoffs passed. The external tokenizer proof was not rerun because its directory variable was unset and its pinned fixture is unchanged | Keep green at every milestone |
