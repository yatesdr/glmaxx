...
|---|---|---|---|---|
| C01 | REVIEW | Corrected NVFP4 manifest ABI, EXL3 source projection, and EXL3 warp-decode gates accepted | Corrective handoffs at `0edfc8d`; no acceptance tokens are present. Prior manifest finding `m7` now has a bit-exact CPU correction and dedicated handoff at `13f0c59`, also awaiting review | Fable re-review of the three r2 handoffs and the narrow indexer-scale correction |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| D01 | PASS | Local format, tests, Clippy, FFI checks, deterministic fixtures, review provenance, and pinned tokenizer proof pass | Latest local run: 230 Rust tests, including exhaustive indexer-scale exponent boundaries and the deterministic integrated cache-lifecycle fixture, passed; all 29 then-present handoffs verified in that full run, and the new indexer-scale handoff separately passes `review-proof`; accepted-review classification also requires the exact candidate, every pinned input hash, any required result path, and a distinct review artifact; external tokenizer was not rerun because its directory variable was unset, and its pinned fixture is unchanged from the prior proof | Keep green at every milestone |
