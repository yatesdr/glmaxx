...
| C03 | REVIEW | Scheduler, prefix leases, active KV IDs, rank uploads, commit, rollback, and ID quarantine form one transaction | `docs/serving-page-transaction-v1.md`; handoff `61eb0b9`. CPU corrections `2f7d0ce`, `14b97a2`, and `2ff0ac1` make scheduler completion, prefix release, and post-selection failure finalization bounded and fail-closed, but do not implement post-success multi-request cleanup or the wider page/rank transaction | Resolve atomicity/finalization and transaction reviews, then implement the bounded cross-component undo log |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| S01 | PASS | CPU scheduler supports bounded continuous batching, weighted tenant fairness, prefix-pending admission, and collective-safe cancellation | Scheduler/serving tests include candidate `2f7d0ce` for retryable C64 completion, `14b97a2` for counted prefix release, and `2ff0ac1` for consuming every selected batch on compile/submit/worker/output failure. All dedicated Fable handoffs await review | Resolve the CPU finalization reviews, then re-prove with the real executor |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| D01 | PASS | Local format, tests, Clippy, FFI checks, deterministic fixtures, review provenance, and pinned tokenizer proof pass | Latest full local run: 239 Rust tests, including selected-step failure finalization, all-or-nothing prefix release and scheduler completion, captured-shape prefill progress, exact asynchronous restore identity, atomic multi-victim HBM admission, and all prior CPU fixtures, passed; all 39 then-present handoffs verified in that full run, and the new finalization handoff separately passes `review-proof`; accepted-review classification also requires the exact candidate, every pinned input hash, any required result path, and a distinct review artifact; external tokenizer was not rerun because its directory variable was unset, and its pinned fixture is unchanged from the prior proof | Keep green at every milestone |
