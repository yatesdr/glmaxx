...
| K04 | REVIEW | Newly sealed runtime pages publish target plus draft sidecar durably and become reusable without restart | Contract candidate `d0a09d7` separates HBM/durable generations, publication leases, live catalog, incremental/restart index, and bounded writes; implementation is intentionally absent. The existing synchronous CPU store fail-stops writes after uncertain publication at `a5019aa` and enforces one writable authority at `535a8d6`; both dedicated reviews are pending | Fable reviews of the online-publication, durable fail-stop, and single-writer handoffs, then the online CPU proof matrix |
| K05 | OPEN | Prefix cache survives restart, corruption, torn journal, cache thrash, pinned pressure, and DCP posture changes end to end | Candidate `d0ac1d3` adds a deterministic integrated CPU lifecycle proof. Corrections through `535a8d6` bind restore identity, make multi-victim admission atomic, make single- and multi-request release atomic, preserve exact pending restore/admission ownership when rollback is blocked, forbid a second durable write after uncertain publication, reject multiple writers, and remove mutation authority from restore workers. All await review, and no model/GPU/direct-I/O lifecycle exists | Preserve the CPU fixture and correction proofs while implementing accepted K03/K04 and C05 paths, then run the integrated cold/warm fault matrix |
