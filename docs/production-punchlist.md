...
| C03 | REVIEW | Scheduler, prefix leases, active KV IDs, rank uploads, commit, rollback, and ID quarantine form one transaction | `docs/serving-page-transaction-v1.md`; handoff `61eb0b9`. CPU correction `2f7d0ce` separately makes request/tenant scheduler batch completion all-or-nothing and retryable on late overflow, but does not implement the wider page/rank transaction | Resolve active-page, scheduler-atomicity, and transaction reviews, then implement the bounded cross-component undo log |
...
| K05 | OPEN | Prefix cache survives restart, corruption, torn journal, cache thrash, pinned pressure, and DCP posture changes end to end | Candidate `d0ac1d3` adds a deterministic integrated CPU lifecycle proof. Correction `dc16273` binds every asynchronous completion to its exact request ID, ordinal, and durable record; candidate `c84da2a` makes multi-victim HBM admission all-or-nothing under pinned pressure. Both corrections await review, and no model/GPU/direct-I/O lifecycle exists | Preserve the CPU fixture and both correction proofs while implementing accepted K03/K04 and C05 paths, then run the integrated cold/warm fault matrix |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| S01 | PASS | CPU scheduler supports bounded continuous batching, weighted tenant fairness, prefix-pending admission, and collective-safe cancellation | Scheduler/serving tests now include candidate `2f7d0ce`: completion is preflighted in fixed C64 arrays, a forced second-row overflow leaves the exact inflight batch retryable, and its dedicated Fable handoff awaits review | Resolve scheduler-atomicity review, then re-prove with the real executor |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| D01 | PASS | Local format, tests, Clippy, FFI checks, deterministic fixtures, review provenance, and pinned tokenizer proof pass | Latest full local run: 237 Rust tests, including all-or-nothing scheduler completion, captured-shape prefill progress, exact asynchronous restore identity, atomic multi-victim HBM admission, and all prior CPU fixtures, passed; all 37 then-present handoffs verified in that full run, and the new scheduler handoff separately passes `review-proof`; accepted-review classification also requires the exact candidate, every pinned input hash, any required result path, and a distinct review artifact; external tokenizer was not rerun because its directory variable was unset, and its pinned fixture is unchanged from the prior proof | Keep green at every milestone |
