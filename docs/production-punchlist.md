...
| C11 | OPEN | Strict four-rank checkpoint loader maps a fit-capable rank set into immutable device arenas and reaches healthy startup | File-backed one-pass rank verification passes at `ee9d16b`; two-phase device-load transaction candidate `737603b` awaits review; no CUDA sink, full-rank proof, or device residency | Fable load-transaction review, CPU/mock proof, then CUDA sink and small-checkpoint smoke |
