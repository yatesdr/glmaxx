...
| K04 | REVIEW | Newly sealed runtime pages publish target plus draft sidecar durably and become reusable without restart | Contract candidate `d0a09d7` separates HBM/durable generations, publication leases, live catalog, incremental/restart index, and bounded writes; implementation is intentionally absent. The existing synchronous CPU store now fail-stops further writes after any uncertain publication error at `a5019aa`, with a dedicated review pending | Fable reviews of `docs/fable-online-prefix-publication-v1-handoff.md` and `docs/fable-durable-store-write-fail-stop-v1-handoff.md`, then the online CPU proof matrix |
| K05 | OPEN | Prefix cache survives restart, corruption, torn journal, cache thrash, pinned pressure, and DCP posture changes end to end | Candidate `d0ac1d3` adds a deterministic integrated CPU lifecycle proof. Corrections `dc16273`, `c84da2a`, `14b97a2`, `6535248`, `bfbe7f4`, and `a5019aa` bind restore identity, make multi-victim admission atomic, make single- and multi-request release atomic, preserve exact pending restore/admission ownership when rollback is blocked, and forbid a second durable write after uncertain publication. All await review, and no model/GPU/direct-I/O lifecycle exists | Preserve the CPU fixture and correction proofs while implementing accepted K03/K04 and C05 paths, then run the integrated cold/warm fault matrix |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| D01 | PASS | Local format, tests, Clippy, FFI checks, deterministic fixtures, review provenance, and pinned tokenizer proof pass | Latest full local run: 247 Rust tests, including fail-stop durable writes after uncertain publication, atomic retryable active-sequence removal, backend event-cancellation and retained-admission fatal propagation, retryable pending restore/admission rollback, deterministic restore-queue saturation cleanup, multi-request terminal/cancellation cleanup, exact C64/MTP6 event staging, selected-step failure finalization, all-or-nothing prefix/scheduler completion, captured-shape prefill, restore identity, HBM admission, and all prior CPU fixtures, passed; all 45 then-present handoffs verified in that full run, and the new durable-store handoff separately passes `review-proof`; accepted-review classification also requires the exact candidate, every pinned input hash, any required result path, and a distinct review artifact; external tokenizer was not rerun because its directory variable was unset, and its pinned fixture is unchanged from the prior proof | Keep green at every milestone |
