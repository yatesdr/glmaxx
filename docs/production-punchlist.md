...
| C03 | REVIEW | Scheduler, prefix leases, active KV IDs, rank uploads, commit, rollback, and ID quarantine form one transaction | Serving integration `f480ef1` makes the active table mandatory and transactional. `271d1f4` now adds canonical successor deltas with changed suffixes, global/rank-local digests, arena/owner validation, and atomic mirror reconstruction. Coordinator/worker delivery, fixed undo, device acknowledgment, and ID quarantine remain absent; dedicated reviews pending | Fable active-page/delta verdicts, then bind delta to immutable step input and four rank receipts |
