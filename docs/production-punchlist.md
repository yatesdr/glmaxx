...
| H05 | OPEN | Graph-captured prefill/decode/MTP buckets pass repeated correctness and timing | CPU candidate `9bdb208` now chunks configured prefill work to the highest-work legal captured entry instead of stalling on a wider uncaptured shape. The audit also proved that the current unique `GraphKey` cannot encode multiple prefill prompt-row buckets for the same sequence/transport key; no CUDA graph has run | Fable review of `docs/fable-prefill-captured-shape-v1-handoff.md`, then design/review the prompt-row graph-key extension before C05/H02 capture/eager comparison |
