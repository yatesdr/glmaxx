...
| K03 | REVIEW | Qualified DRAM↔NVMe restore/eviction works under pressure without blocking decode | Contract candidate `69895e0` defines aligned 493/501-block direct extents, registered-buffer io_uring ownership, restore dedup/cancel, shared catalog epochs, tier scheduling, segment cleaning, and matched decode-isolation evidence; implementation is intentionally absent. The retained blocking restore workers now use read-only snapshot handles at `535a8d6`, but still block and retain private catalogs | Fable reviews of `docs/fable-direct-tier-io-v1-handoff.md` and `docs/fable-durable-store-single-writer-v1-handoff.md`, then direct-format/CPU state proof |
| K04 | REVIEW | Newly sealed runtime pages publish target plus draft sidecar durably and become reusable without restart | Contract candidate `d0a09d7` separates HBM/durable generations, publication leases, live catalog, incremental/restart index, and bounded writes; implementation is intentionally absent. The existing synchronous CPU store fail-stops writes after uncertain publication at `a5019aa`, enforces one writable authority at `535a8d6`, and corrections through `b097703` enforce one no-write dedup/MTP-upgrade matrix across prefix, residency, file append, and journal replay; dedicated reviews are pending | Fable reviews of the online-publication and retained-store correction handoffs, then the online CPU proof matrix |
| K05 | OPEN | Prefix cache survives restart, corruption, torn journal, cache thrash, pinned pressure, and DCP posture changes end to end | Candidate `d0ac1d3` adds a deterministic integrated CPU lifecycle proof. Corrections through `8612ec3` bind restore identity, make multi-victim admission atomic, make single- and multi-request release atomic, preserve exact pending restore/admission ownership when rollback is blocked, forbid a second durable write after uncertain publication, reject multiple writers, remove mutation authority from restore workers, reject same-key logical byte conflicts before append and during replay, retain exact durable revisions/MTP capability, and reject any corrupt complete journal tail while still ignoring a genuinely short crash fragment. All await review, and no model/GPU/direct-I/O lifecycle exists | Preserve the CPU fixture and correction proofs while implementing accepted K03/K04 and C05 paths, then run the integrated cold/warm fault matrix |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| D01 | PASS | Local format, tests, Clippy, FFI checks, deterministic fixtures, review provenance, and pinned tokenizer proof pass | Latest full local run: 254 Rust tests, including complete journal-tail corruption rejection with short-tail recovery, shared no-write durable dedup/MTP upgrade/collision-safe replay, two-rank atomic MTP registration, prefix-index/owner-rank residency coherence, same-key logical collision rejection, exclusive durable writer/read-only restore snapshots, fail-stop durable writes after uncertain publication, atomic retryable active-sequence removal, backend event-cancellation and retained-admission fatal propagation, retryable pending restore/admission rollback, deterministic restore-queue saturation cleanup, multi-request terminal/cancellation cleanup, exact C64/MTP6 event staging, selected-step failure finalization, all-or-nothing prefix/scheduler completion, captured-shape prefill, restore identity, HBM admission, and all prior CPU fixtures, passed; all 50 then-present handoffs verified in that full run, and the new journal-tail handoff separately passes `review-proof`; accepted-review classification also requires the exact candidate, every pinned input hash, any required result path, and a distinct review artifact; external tokenizer was not rerun because its directory variable was unset, and its pinned fixture is unchanged from the prior proof | Keep green at every milestone |
