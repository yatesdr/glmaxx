...
| C02 | REVIEW | Immutable row input carries prompt IDs/positions, sampling tuple, seed/RNG, MTP posture, limits, and page-delta binding to all ranks | CPU ABI `d1f9825` hashes exact row/prompt/sampling state, distinguishes configured/effective MTP depth, validates the sampling collective, enforces 1M/output bounds, and binds the canonical page-delta digest. Serving construction, worker delivery/receipts, and output RNG commit remain absent | Adversarial CPU ABI review, then four-rank envelope/consensus and serving construction |
| C03 | REVIEW | Scheduler, prefix leases, active KV IDs, rank uploads, commit, rollback, and ID quarantine form one transaction | Serving integration `f480ef1` makes the active table mandatory and transactional. `271d1f4` adds canonical successor deltas and atomic bounded mirrors; `d1f9825` binds the exact reservation delta into immutable step input. Admission/removal/commit delivery, fixed undo, device acknowledgment, and ID quarantine remain absent | Review current CPU pieces, then rank mirror initialization, mutation receipts, post-output commit, and quarantine |
...
| S04 | REVIEW | Exact probabilistic parameters and deterministic seed/RNG state reach rank execution and responses | Sampling/RNG candidate `7c71818`; StepInput CPU ABI `d1f9825` carries exact float bits, top-k, seed, and pre-step counter and validates greedy/top-k/mass route identity. Backend still correctly rejects non-greedy because rank delivery and output-counter commit are absent | Review both contracts, integrate worker/serving counter consensus, then remove the rejection |
