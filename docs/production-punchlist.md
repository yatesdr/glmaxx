...
| K05 | OPEN | Prefix cache survives restart, corruption, torn journal, cache thrash, pinned pressure, and DCP posture changes end to end | Candidate `d0ac1d3` adds a deterministic integrated CPU lifecycle proof. Corrections through `8fb3adf` bind restore identity, make admission/release transactions atomic, fail-stop uncertain writes, reject multiple writers and same-key logical conflicts, retain exact revisions/MTP capability, reject any corrupt complete journal tail, and truncate/sync only a validated short crash fragment before resumed append while read-only snapshots remain nonmutating. All await review, and no model/GPU/direct-I/O lifecycle exists | Preserve the CPU fixture and correction proofs while implementing accepted K03/K04 and C05 paths, then run the integrated cold/warm fault matrix |
...
| ID | State | Required outcome | Current evidence / blocker | Next gate |
|---|---|---|---|---|
| D01 | PASS | Local format, tests, Clippy, FFI checks, deterministic fixtures, review provenance, and pinned tokenizer proof pass | Latest full local run: 254 Rust tests, including validated short-tail truncation/sync followed by recoverable append, complete journal-tail corruption rejection, shared no-write durable dedup/MTP upgrade/collision-safe replay, two-rank atomic MTP registration, prefix-index/owner-rank residency coherence, same-key logical collision rejection, exclusive durable writer/read-only restore snapshots, fail-stop durable writes after uncertain publication, atomic retryable active-sequence removal, backend event-cancellation and retained-admission fatal propagation, retryable pending restore/admission rollback, deterministic restore-queue saturation cleanup, multi-request terminal/cancellation cleanup, exact C64/MTP6 event staging, selected-step failure finalization, all-or-nothing prefix/scheduler completion, captured-shape prefill, restore identity, HBM admission, and all prior CPU fixtures, passed; all 51 then-present handoffs verified in that full run, and the new torn-journal handoff separately passes `review-proof`; accepted-review classification also requires the exact candidate, every pinned input hash, any required result path, and a distinct review artifact; external tokenizer was not rerun because its directory variable was unset, and its pinned fixture is unchanged from the prior tokenizer proof | Keep green at every milestone |
