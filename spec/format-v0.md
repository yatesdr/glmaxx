...
5. target and MTP-draft HBM page geometry;
6. the DRAM/NVMe target record and draft sidecar.
...
Within the logical unswizzled row, two E2M1 values are packed per byte.
Nibble order SHALL match the selected PTX/CUTLASS conversion and be recorded
as `nibble_order` in metadata.

The exact TMA/value-plane permutation is identified by `value_layout_id`.
Layout ID zero is invalid.

**OPEN NVFP4-L1:** the first selected SM120 kernel and CUTLASS revision MUST
freeze the exact value permutation and nibble order before a rank file can
be called direct-layout compatible.
...
**OPEN NVFP4-L2:** freeze the explicit natural-index to payload-offset
formula and the exact CUTLASS source revision before format minor 2 is
frozen.
...
The draft sidecar fragment is 23,552 bytes per logical page. Across the
model limit it consumes 385,875,968 bytes = 0.359375 GiB aggregate, or
0.08984375 GiB per DCP4 rank before tail slack.

Target and draft page tables are distinct but SHALL use the same logical page
ordinal and owner. An MTP-capable page commit SHALL publish matching target
and draft generations atomically.
...
An MTP-capable sealed prefix SHALL also store one draft record occupying
seven 4-KiB blocks:

```text
4,096-byte header
23,552-byte payload
1,024-byte zero padding
= 28,672 bytes
```
...
```text
[draft_layer=0][token=0..63][record_byte=0..367]
```

A target record MAY exist without a draft record and is then MTP0-only. A
draft record MUST NOT exist or attach without its paired target record.

## 23. Sealed tier headers

The target and draft headers are 4,096 bytes:

| Offset | Bytes | Field |
|---:|---:|---|
| 0 | 8 | target magic `G5KVPAG0`; draft magic `G5MTPG0` |
| 8 | 2 | major `0` |
| 10 | 2 | minor `2` |
...
| 152 | 4 | target layer count `78`; draft layer count `1` |
| 156 | 4 | token record bytes; `368` |
| 160 | 8 | target payload `1,837,056`; draft payload `23,552` |
...
A draft header SHALL contain the exact target page key at `[244,276)`.
Target and draft headers SHALL use different content namespace hashes and
therefore different page keys.
...
- DCP4 owner mapping;
- target/draft page-table and physical HBM layout;
...
An MTP-capable write SHALL complete and flush both target and draft records,
then publish both index entries in one generation. An incomplete draft write
MAY leave the independently valid target unpublished or publish it later as
MTP0-only; it MUST NOT publish an orphan draft entry.
...
- target/draft sidecar pairing, atomic commit, and orphan rejection;
...
3. Explicit Blackwell scale swizzle offset formula.
