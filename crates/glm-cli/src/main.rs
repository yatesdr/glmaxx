...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match arguments.get(1).map(String::as_str) {
        ...
        Some("serving-host-profile") => {
            if arguments.len() != 6 {
                return Err(
                    "serving-host-profile requires evidence-dir source-commit warmups iterations"
                        .into(),
                );
            }
            let evidence = arguments
                .get(2)
                .ok_or("serving-host-profile requires an evidence directory")?;
            let source_commit = arguments
                .get(3)
                .ok_or("serving-host-profile requires a source commit")?;
