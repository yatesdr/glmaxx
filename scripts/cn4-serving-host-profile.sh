...
readonly run="$(${source}/scripts/new-evidence-run.sh \
    "${root}/evidence" "serving-host-profile-${expected_commit:0:7}")"
...
cp "$0" "${run}/run.sh"
printf '%s\n' \
    "docker run --rm --network none --cpuset-cpus 0-15 --env NVIDIA_VISIBLE_DEVICES=void ${image} cargo build --locked --offline --release -p glm-cli --bin glmaxx; glmaxx serving-host-profile RESULT/profile ${expected_commit} 100 1000" \
    >"${run}/command.txt"
...
sha256sum "${profile}/serving-host-profile.json" \
    >"${run}/result-sha256.txt"
jq -n \
    --arg schema "glmaxx.cn4-serving-host-profile.v1" \
    --arg source_commit "${expected_commit}" \
    --arg raw_sha256 "$(sha256sum "${profile}/serving-host-profile.json" | cut -d' ' -f1)" \
    --slurpfile profile "${profile}/serving-host-profile.json" \
    '{
        schema: $schema,
        source_commit: $source_commit,
        raw_sha256: $raw_sha256,
        gpu_launched: false,
        claim: "synthetic Rust host-path overhead only",
        profile: $profile[0]
    }' >"${run}/summary.json"
